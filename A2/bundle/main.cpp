    ...
    for (int r{ 0 }; r < world.height; ++r) {
        for (int c{ 0 }; c < world.width; ++c) {
            ...
            for (int i{ 0 }; i < world.sampler->getNumSamples(); ++i) {
                ...
                pixelPoint.y = c - 0.5f * world.height + samplePoint.y;
                ray.d = rayDirection(pixelPoint);

                for (auto const& obj : world.scene) {
                    obj->hit(ray, sr_temp);
                    if (sr_temp.t > sr_closest.t) {
                        sr_closest.t = sr_temp.t;
                    }
                }
                pixelAverage += sr_closest.t;
            }
            world.image.push_back({ pixelAverage.r * avg, pixelAverage.g * avg, pixelAverage.b * avg });
        }
    }
}
...
int main()
{
    ...
    world.scene.push_back(std::make_shared<Sphere>(atlas::math::Point{ 64, 64, 0 }, 128.0f));
    world.scene[0]->setColour({ 1, 0, 0 });

    world.scene.push_back(std::make_shared<Sphere>(atlas::math::Point{ 128, 128, 64 }, 64.0f));
